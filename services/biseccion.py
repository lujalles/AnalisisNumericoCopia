...
def biseccion(f, a, b, es, imax, xi, xf):
    ...
    bisec_table.append(["--", "--", "--", "--", "--", "--", "--", "--"])
    bisec_table.append([i, a, b, f(a), f(b), m, f(m), "--"])
    ...
    while ea > es and i <= imax:
        ...
        fmv.append(f(m))
        bisec_table.append([i, a, b, f(a), f(b), m, f(m), ea])

    x_vals = np.linspace(xi, xf, 100)
    y_vals = [f(val) for val in x_vals]

    with PdfPages('reporte_biseccion.pdf') as pdf:
        resumen = (
            "REPORTE DEL MÉTODO DE BISECCIÓN\n\n"
        ...
        df = pd.DataFrame(bisec_table[1:], columns=["Iteración", "a", "b", "f(a)", "f(b)", "m", "f(m)", "e abs"])
        rows_per_page = 20
        ...
        for i in range(0, len(df), rows_per_page):
            ...
            table.scale(1.2, 1.5)
            ax.set_title(f"Método de Bisección - Página {i // rows_per_page + 1}", fontsize=12, pad=20)
            pdf.savefig()
            plt.close()
